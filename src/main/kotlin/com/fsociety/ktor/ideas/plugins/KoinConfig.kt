...
val appModule = module {
    ...
    // Kafka Configuration
    single<String>(qualifier = named("bootstrapServers")) { "localhost:9093" }
    single<String>(qualifier = named("userTopic")) { "user-topic" }
    single<String>(qualifier = named("messageTopic")) { "message-topic" }
    single<String>(qualifier = named("consumerGroupId")) { "ktor-ideas-consumer-group" }

    // Kafka Producer
    single<Producer<String, User>> {
        createProducer(
            bootstrapServers = get(qualifier = named("bootstrapServers")),
    ...
    // Kafka Consumer
    single<Consumer<String, KafkaMessage>> {
        createConsumer(
            bootstrapServers = get(qualifier = named("bootstrapServers")),
            groupId = get(qualifier = named("consumerGroupId")),
            deserializer = KafkaMessage.KafkaMessageDeserializer()
        )
    }

    single<KafkaConsumer<KafkaMessage>> {
        KafkaConsumer.createForKafkaMessage(
            kafkaMessageConsumer = get(),
            messageTopic = get(qualifier = named("messageTopic"))
        )
    }
